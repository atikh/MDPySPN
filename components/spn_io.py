...
def write_to_event_log(simulation_time, token_ids, event, transition, spn):
    ...
    try:
        ...
        with open(file_path, "a", newline="") as event_log:
            ...
            dimension_totals = {'energy': 0.0, 'waste': 0.0}
                ...
                if hasattr(trans, 'dimension_table'):
                    for dimension, value in trans.dimension_table.items():
                        if dimension in dimension_totals:
                            dimension_totals[dimension] += value

            # Write the aggregated values to the event log
            writer.writerow([
                str(round(simulation_time, 2)),  # Time Stamp
                str(token_ids),  # Token ID
                event,  # Event Name
                round(dimension_totals['energy'], 2),  # Cumulative Energy Value
                round(dimension_totals['waste'], 2)  # Cumulative Waste Value
            ])
